    ...
    <div
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                <!-- Game thumbnail -->
                <GameThumbnail :thumbnail-url="huntItem.custom_thumb" :background-url="huntItem.url_background"
                    :alt="huntItem.item" size="sm" @image-error="handleImageError" />
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <input type="number" v-model="localItem.wager"
                        class="w-24 px-2 py-1 border rounded bg-brown-dark text-gold" @input="updateItem" />
                </div>

                <!-- Result input -->
                <div class="flex items-center gap-2">
                    <label class="text-sm text-gold">Result:</label>
                    <input type="number" v-model="localItem.result"
                        class="w-24 px-2 py-1 border rounded bg-brown-dark text-gold" @input="updateItem" />
                </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <input type="checkbox" v-model="localItem.bonus"
                ...
                <div class="flex items-center gap-2">
                    ...
                    <input type="checkbox" v-model="localItem.super_bonus"
                        class="form-checkbox h-5 w-5 text-orange-600 rounded border-orange-400 focus:ring-orange-500"
                ...
                <!-- Remove button -->
                <ActionButton icon="trash" variant="danger" size="sm" @click="emit('remove', props.huntItem.id)" />
            </div>
        </div>
        ...
        <div v-if="showBalanceModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-brown-dark border border-gold rounded-lg p-6 w-96 shadow-xl">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" v-model="currentBalance"
                        class="w-full px-3 py-2 border rounded bg-brown-light text-gold"
                        placeholder="Enter current balance" />
                </div>
                <div class="flex justify-end gap-3">
                    <button @click="cancelBalanceUpdate()"
                        class="px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700">
                        Cancel
                    </button>
                    <button @click="saveBalanceUpdate()"
