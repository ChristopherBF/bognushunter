        ...
        <div class="flex justify-between items-center">
            <div class="flex flex-col">
                ...
                <div class="flex items-center gap-4 mt-1">
                    <span class="text-sm text-streamer-text-secondary">Starting Balance: {{ event.starting_balance
                        }}$</span>
                    <span class="text-sm text-streamer-text-secondary">Current Balance: {{ event.current_balance
                        }}$</span>
            ...
            <div class="flex space-x-2 items-center">
                <button v-if="event.open" @click.stop="$emit('close-event', event.id)"
                    ...
                    :disabled="!event.open" title="Close Event">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <button @click="$emit('view-suggestions', event.id)"
                ...
                <button @click="$emit('view-summary', event.id)"
                    class="px-3 py-1 bg-orange-700 text-gold text-sm rounded hover:bg-orange-800 hover:text-gold shadow transition-colors duration-150 flex items-center gap-1">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <button @click="$emit('view-hunt-list', event.id)"
                    class="px-3 py-1 bg-orange-700 text-gold text-sm rounded hover:bg-orange-800 hover:text-gold shadow transition-colors duration-150 flex items-center gap-1">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <button @click="$emit('share-link', event.id)"
                    class="px-3 py-1 bg-orange-700 text-gold text-sm rounded hover:bg-orange-800 hover:text-gold shadow transition-colors duration-150 flex items-center gap-1">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
